...
def audit_data_leakage(dataset_root: Path, val_split: float = 0.15, seed: int = 42) -> Dict:
    ...
    if max(counts) / max(min(counts), 1) > 3:
        ...
        baseline = 100.0 * max(counts) / n_frames
        print(f"\n  ** CLASS IMBALANCE ** {max(counts)}:{min(counts)} ({max(counts)/max(min(counts),1):.1f}x)")
...
def run_kfold_cv(
    ...
    label_counts = Counter(int(x) for x in group_labels)
...
EXTERNAL_TEST_QUERIES = {
    "healthy": ["quinoa bowl meal prep", "fresh fruit smoothie recipe", "steamed vegetables dinner plate"],
    "unhealthy": ["deep fried mozzarella sticks", "loaded nachos cheese", "chocolate cake frosting slice"],
}
